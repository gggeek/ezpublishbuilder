...
function run_update_ci_repo( $task=null, $args=array(), $cliopts=array() )
{
    ...
    $files = pakeFinder::type( 'file' )->maxdepth( 0 )->in( eZPCPBuilder::getSourceDir( $opts, 'legacy' ) . '/' . $changelogdir );
    if ( count( $files ) != 1 )
    {
    ...
    */
    ...
    static function getPreviousRevision( $prevName, $repo, $opts )
    {
        ...
        if ( is_array( $out ) && isset( $out['builds'] ) )
        {
            ...
            if ( $previousbuild )
            {
                $out = eZPCPBuilder::jenkinsCall( 'job/' . $jenkinsJobsName . '/' . $previousbuild . '/api/json', $opts );
